    ...
    <div class="container-fluid">
        ...
        <div class="form-box">
            ...
            <div class="form-content">
                <form action="{{ route('admin.products.images.save') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                        ...
                        <table class="table table-bordered text-center align-middle">
                            ...
                            <tbody>
                                ...
                                <tr class="image-row" data-id="{{ $image->image_id }}">
                                    ...
                                    <td>
                                        <input type="hidden" name="images[{{ $image->image_id }}][image_id]" value="{{ $image->image_id }}">
                                        <select name="images[{{ $image->image_id }}][product_id]" class="form-control">
                                            @foreach($products as $product)
                                                <option value="{{ $product->product_id }}" {{ $product->product_id == $image->product_id ? 'selected' : '' }}>
                                                    {{ $product->title }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <td>
                                        <img src="{{ asset($image->image_path) }}" width="80" class="mb-1"><br>
                                        <input type="file" name="images[{{ $image->image_id }}][image]" class="form-control mt-1">
                                    </td>
                                    <td><input type="text" name="images[{{ $image->image_id }}][alt_text]" class="form-control" value="{{ $image->alt_text }}"></td>
                                    <td><input type="number" name="images[{{ $image->image_id }}][sort_order]" class="form-control" value="{{ $image->sort_order }}"></td>
                                    <td>
                                        <select name="images[{{ $image->image_id }}][is_primary]" class="form-control">
                                            <option value="1" {{ $image->is_primary ? 'selected' : '' }}>نعم</option>
                                            <option value="0" {{ !$image->is_primary ? 'selected' : '' }}>لا</option>
                                        </select>
                                    </td>
                                    <td>
                                        <button type="button" class="btn btn-danger remove-image" data-id="{{ $image->image_id }}">حذف</button>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
