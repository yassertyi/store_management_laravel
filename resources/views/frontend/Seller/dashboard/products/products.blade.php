...
@section('page_title', 'إدارة منتجاتك')
    ...
    <div class="container-fluid">
        ...
        <div class="form-box">
            <div class="form-title-wrap d-flex justify-content-between align-items-center">
                <h3 class="title">قائمة منتجاتك</h3>
                <a href="{{ route('seller.products.create') }}" class="theme-btn theme-btn-small">
                            ...
                            @foreach ($products as $product)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <span class="badge text-bg-danger">معلق</span>
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        @foreach ($product->variants as $var)
                                            <div>{{ $var->name }} - {{ number_format($var->price, 2) }} - {{ $var->stock }}</div>
                                        @endforeach
                                    </td>
                                    <td>
                                        <a href="{{ route('seller.products.show', $product) }}" class="theme-btn theme-btn-small me-2">
                                            <i class="la la-eye"></i>
                                        </a>
                                        <a href="{{ route('seller.products.edit', $product->product_id) }}" class="theme-btn theme-btn-small me-2">
                                            <i class="la la-edit"></i>
                                        </a>
                                        <form action="{{ route('seller.products.destroy', $product->product_id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
